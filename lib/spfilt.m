...
% Chiease：该函数视线性和非线性空间滤波。
%   F = SPFILT(G, TYPE, M, N, PARAMETER)表示对图像G进行空间滤波，使用一个尺寸为M * N的指定类型滤波器。
%     F = SPFILT(G, 'amean', M, N)       算术平均
%     F = SPFILT(G, 'gmean', M, N)       几何平均
%     F = SPFILT(G, 'hmean', M, N)       调和均值
%     F = SPFILT(G, 'chmean', M, N, Q)   反调和平均
%     F = SPFILT(G, 'median', M, N)      中值滤波器
%     F = SPFILT(G, 'max', M, N)         最大值滤波器
%     F = SPFILT(G, 'min', M, N)         最小值滤波器
%     F = SPFILT(G, 'midpoint', M, N)    中点滤波器
%     F = SPFILT(G, 'atrimmed', M, N, D) 顺序平衡均值
%
%   默认值 M = N = 3,
%   Q = 1.5, and D = 2. 
